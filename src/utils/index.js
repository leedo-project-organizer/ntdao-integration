...
if (network == 'matic') {
  contractAddr = '0x94576423d85b47575BBA515a1F328A265e6318e6'
...
const getContract = async () => {
  provider = new ethers.providers.AlchemyProvider.getWebSocketProvider(network, alchemyId)
  rawWallet = ethers.Wallet.fromMnemonic(readMnemonic(), currentPath)
...
const _csvToArray = async (filepath, rtnArry) => {
...
const csvToArray = async (filepath) => {
  let rtn = []
  await _csvToArray(filepath, rtn)
  return rtn
}

const getSimpleArray = async (filepath) => {
  let rtn = []
  fs.createReadStream(filepath)
  .pipe(csv())
  .on('data', (row) => {
    rtn.push(Object.values(row)[0])
  })
  .on('end', () => {
    console.log('CSV file successfully processed');
  });
...
const batchSend = async (filepath) => {
  let r = getSimpleArray(filepath)
  let batch = 200;
}
